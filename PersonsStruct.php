...
//$_GET['avtor']='7';
	...
	if($_GET['avtor'] !='all'){//если мы выводим структуру какого либо определенного автора, то просчитываем ее
		...
		$query = "SELECT * FROM  InfoTable "; 
			//echo "ппппппппппппппппппппппппппппппппп";		
		...
		//echo "Вытаскиваем из БД arrAvtAll - ".$row['AvtorArray'];
...
//echo "gggg1";			
			...
			echo $row['AvtorArray'];
			$arrAvtAll = unserialize($row['AvtorArray']);	
			echo "fgfsgfdsgs";
			var_dump($arrAvtAll);
			*/
			  ...
			  if(!file)
				...
				//  echo "nn-".filesize("AvtorArray.txt")."-nn";
				//  echo "<br />999999";
				 // $buff = fread ($file,999999);
				$buff = file_get_contents("AvtorArray.txt");
				//  echo $buff;
				  //print $buff;
				  $arrAvtAll = unserialize($buff);	
				}
	...
	//if(!$arr){ $arr[] }
		...
		//var_dump($v);
			...
			//if(count($spCheckArr) >2){
				...
				if($_SESSION['Guest_id']['id_user'] != $AdminID){//выводим количество нередактированных речей
					if($spCheckArr['2'] >0){ $col="red";}else{$col="purple;";}
					$allSp .= "<b style='color:".$col."; font-size:15px;'>".$spCheckArr['2']." | ".$spCheckArr['1']."</b>";
				}
			//}
			$avtorStr = "<div class='AvtTD' align='center' ".$stlNeedAvt." ".$onclick." id='WhI-".$k."'>
				...
				if($pos == 'first'){ $style = "style='height:40px; margin-left:50%;  background:url(img/structL.png) repeat; '"; //border:1px solid red;
				}elseif($pos == 'last'){ $style = "style='height:40px; margin-right:50%; background:url(img/structR.png) repeat;  background:url(img/struct.png) no-repeat; background-position:100% 0px;'";
				}elseif($pos == 'mid'){ $style = "style='height:40px; background:url(img/structM.png) repeat; background-position:50% 0px; '";
				}elseif($pos == 'edinstv'){ $style = "style='height:20px;  background-position:50% 0px;'";  //background:url(img/struct1.png) no-repeat;
				}		
...
<?if($_SESSION['Guest_id']['id_user'] == $AdminID){	?>
	<script>//показ формы поиска автора по имени или описанию	
				var opt = new Array(); //параметры
				opt.IndexZ = 3 //IndexZ
				opt.fixed = 'fix'; //означает, что она зафиксирована экране
				opt.left = 15;
				opt.top = 15;
				var elemSearch = CreateElementOnScreen(760,'',opt);
				elemSearch.innerHTML = "<span onclick='deletElem(this)' id='delme' style='cursor:pointer; float:right; margin: -10px -5px 0 0;' > <span style='color:black;'>_</span><b>x</b><span style='color:black;'>_ </span></span><br />";  //закрывание этого списка
				elemSearch.id = 'MenuSearch';
				elemSearch.innerHTML +="<b>Поиск по имени:</b><br />";
				elemSearch.innerHTML +="<input type='text' size='25' value='' /><button  onclick='FindAvtorsByName(this)' >ок</button>";
				elemSearch.innerHTML +="<br /><b>Поиск по описанию:</b><br />";
				elemSearch.innerHTML +="<input type='text' size='25' value='' /><button  onclick='FindAvtorsByDescr(this)' >ок</button>";	
				//elemSearch.innerHTML +="<div id='here'></div>";		
				elemSearch.innerHTML +="<br /><select id='here'  name='1111'  >";//style='z-index: 500; position:absolute;' 
/*
					//elemMenu.innerHTML += "<div id='idWh-"+id+"' onclick='ShowFotoMen(this)' style='text-decoration:underline; cursor:pointer;'>Прикрепить фото</div>";
					elemSearch.innerHTML +="<b>Админское:</b><br /> id автора - "+id+"<br />";
					elemSearch.innerHTML +="<div id='FA-"+id+"'  style='text-decoration:underline; cursor:pointer;'><span onclick='InsertFormAllias(this)' >сделать его псевдонимом</span></div>";
					elemSearch.innerHTML +="<div id='FP-"+id+"'  style='text-decoration:underline; cursor:pointer;'><span onclick='InsertFormPodch(this)' >подчинить другому автору</span></div>";
				elemSearch.innerHTML +="<div id='FD-"+id+"'  style='text-decoration:underline; cursor:pointer;'><span onclick='InsertFormDescr(this)' >поменять описание</span></div>";
					elemSearch.innerHTML +="<div onclick='MakePresident(\""+id+"\")' style='text-decoration:underline; cursor:pointer;'>сделать его президентом</div>";
					elemSearch.innerHTML +="<div onclick='DelAutor(\""+id+"\")' style='text-decoration:underline; cursor:pointer;'>удалить вместе с речами</div>";
					elemSearch.innerHTML +="<br />";
				elemSearch.innerHTML += "<div id='idStruc-"+id+"' onclick='ShowSpeechOfPerson(this)' style='text-decoration:underline; cursor:pointer;'>Показать речь и высказывания</div>";
				elemSearch.innerHTML += "<div onclick='ShoseCtructure(\""+id+"\")' style='text-decoration:underline; cursor:pointer;'>Показать подчиненность</div>";
	*/			
				document.body.appendChild(elemSearch);		
				
				
				function FindAvtorsByName(obj){
					var obj1 = obj.previousSibling; //obj1 = obj1.previousSibling;
					var name = obj1.value;
					//alert("id_who-"+id_who+" id_act_who-"+id_act_who);
					//alert("имя автора-"+name);
					FindInBD('name',name);
					//var res =  ManagerOfStructure(id_who,"MakeDescr",'',descr);
				}
				function FindAvtorsByDescr(obj){
					var obj1 = obj.previousSibling; //obj1 = obj1.previousSibling;
					var descr = obj1.value;
					//alert("id_who-"+id_who+" id_act_who-"+id_act_who);
					//alert("описание-"+descr);
					FindInBD('descr',descr);
					//var res =  ManagerOfStructure(id_who,"MakeDescr",'',descr);
				}
				
				function FindInBD(by,name){//поиск в базе данных по имени или описанию
					//вытаскиваем всех авторов у которых один из имен начинается на эти буквы
					var arr;
					$.ajax({
					  async: false, 
					  url: 'blocks/dinamic_scripts/Find_Avtors_By_ND.php',
					...
					if(arr){
						...
						for(var i=0; i<arr.length; i++){
							...
							var opt = document.createElement('option');
							opt.value = arr1[0];
							opt.innerHTML = arr1[0]+" | "+arr1[1]+" | "+arr1[2];
							Sel.appendChild(opt);
						}
					}
				}
